...
from services.utils import PARENT_FOLDER_ID, SHEETS_ID, SHEETS_RANGE, SHEETS_RANGE_WIDTH, Headers_Map, State_Keys_Map, get_doc_url, get_pdf_url, get_sheet_column_index, update_client_list
...
def process_or_get_images(new_files_list, section_name, existing_links):
    ...
    old_links_quantity = len(existing_links) if existing_links else 0
    ...
    if new_files_list: 
        for idx, img_file in enumerate(new_files_list):
            ...
            _, link = upload_image_to_drive(drive_service, img_file, nome_arquivo, folder_id=State_Keys_Map.DRIVE_ID.st_state)
...
def render_diagnosis_tab():
    ...
    if st.button("📄 Salvar e Gerar Docume  nto de Diagnóstico", type="primary", use_container_width=True):
            ...
            with st.spinner("Conectando ao Google Workspace..."):
                try:
                    ...
                    if client_folder_id is None:
                        client_folder_id, diagnosis_doc_id = create_drive_folder(drive_service, State_Keys_Map.CLIENT.st_state, PARENT_FOLDER_ID)
                    st.session_state[State_Keys_Map.DRIVE_ID.value] = client_folder_id
                    ...
                    st.error(f"Erro ao processar informações: {e}")
                ...
                try:
                    ...
                    diagnosis_doc_id = create_update_diagnostic_doc(
                        drive_service, State_Keys_Map.CLIENT.st_state, data_for_docx, diagnosis_doc_id
                    )
                except Exception as e:
                    st.error(f"Erro ao gerar o documento no Google: {e}")
                ...
                try:                        
                    ...
                    st.error(f"Erro ao salvar informações no Google Sheets: {e}")
